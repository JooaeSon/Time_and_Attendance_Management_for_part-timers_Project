...
//        alert(end);
        ...
        $.ajax({
            url:"./generateAllScueduleData.do",
            method:"post",
            data:{
...
//            	alert(JSON.stringify(msg));
                $.each(msg,function(element){
                	...
                	var calendar = findCalendar(String(msg[element].calendarId)); 
//                	alert(msg[element].calendarId);
                  if (calendar) {
                	 ...
                	 cal.createSchedules([{
    	        		 id : msg[element].id,
    	                 calendarId :String(msg[element].calendarId),
...
function saveScheduleByOwner(){
	var checkboxBtn;
        if($("#reviseable").is(":checked")){
        	checkboxBtn='Y';
        	var conf = confirm("직원/매니저(이)가 스케쥴을 변경 할 수 있습니다 일정을 생성하시겠습니까?","");
        }else{
        	checkboxBtn='N';
        	var conf = confirm("직원/매니저(이)가 스케줄을 변경 할 수 없습니다. 일정을 생성하시겠습니까?","");
        }
// 	alert(checkboxBtn);
        
    if(conf){
     	var wholeDate = $("#renderRange").text().split("~");
     	var start_month=wholeDate[0].trim().substring(0,4)+"-"+wholeDate[0].trim().substring(5,7);
     	var end_month= wholeDate[1].trim().substring(0,4)+"-"+wholeDate[1].trim().substring(5,7);
     	location.href="./updateReviseable.do?chk="+checkboxBtn+"&start_month="+start_month+"&end_month="+end_month;
    }else{
    	alert("일정 생성을 취소하였습니다. 수정 후 저장해 주세요.");
    }
...
// set calendars // 이거 추가할 수 있도록 해야하나? 얘기좀 해봐야게땅,,,ㅎ 
(function() {

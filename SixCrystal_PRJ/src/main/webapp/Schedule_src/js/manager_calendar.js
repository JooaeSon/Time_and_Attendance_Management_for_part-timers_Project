...
//    var readOnlyFlag = false;
    var readOnlyFlag = ($("#flag").val()=='D'||$("#flag").val()==null)?false:true;
    var aTagFlag = ($("#flag").val()=='Y')?true:false;
...
//            console.log('clickSchedule', cal.getOptions());
//            console.log('clickSchedule', cal.getOptions().isReadOnly);
...
//            let userId='rlghd153';
            let userId_title=e.schedule.title
//            alert(userId_title);
            if(userId_title==nowUser){
            	alert("같음!!"+userId_title+" : "+nowUser);
...
//            alert(start);
        ...
        'beforeDeleteSchedule': function(e) {
            let userId_title=$(".tui-full-calendar-schedule-title").text();
        	   if(userId_title==nowUser){
...
//                	 var calendar = CalendarList[rank-1]||"\""+CalendarList[rank-1]+"\"";
                	 var calendar = CalendarList[rank-1];
...
function exchange(userId,id,calenderId,start){
	
	ajaxExchange(userId,id,calenderId,start);
	$("#exchangeData").modal();
...
//			var seletedId;
			var html="";
			  html = "<div class='form-group' id='firstdiv'>";
...
//	        	  
	          ...
	          ///
	         var html2;
	          html2 = "<div class='form-group'>";///지워야될 수도,,!?
	          html2 += "<label for='schedule_time' >선택한 직원의 스케쥴만 고고!! </label>";
	          html2 += "<select name='emp_schedule' id='employee_schedule' class='form-control'>";
	          html2 += "<option value='' disabled selected hidden>--선택--</option>";
...
//		        		  if(key==seletedId){
			        	  $.each(value,function(k,v){
			        		  html2 += "<option value='"+v.id+" "+v.start+" "+v.end+"'>"+"START: "+v.start+" ~ END: "+v.end+"</option>";//여기서 for each를 써야할거같은데?
...
//			        	  html += "<option value="+v.id+" "+v.start+" "+v.end+">"+"START: "+v.start+" ~ END: "+v.end+"</option>";//여기서 for each를 써야할거같은데?
		          ...
		          html2 += "<input class='btn btn-success' type='button' value='교환하기' onclick='exchangeData()'>";
		          html2 += "<input class='btn btn-success' type='button' value='던지기' onclick='throwData()'>";
		          html2 += "<button type='button' class='btn btn-default' data-dismiss='modal'>닫기</button>";
		          html2 +="</div>";
		          $("#firstdiv").append(html2);
	          });
